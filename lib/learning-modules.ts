 ...
 * Path: lib/learning-modules.ts
 * Repo: github.com/thomaspeterkueper/SolarScienceFoundation/blob/main/lib/learning-modules.ts
 * Name: learning-modules - fetch and normalise KXF learning modules export
 * Version: 0.2.0
 * Created: 2026-07-02
 * Modified: 2026-07-04T10:00:00+02:00
 * Depends: components/LearnMap/types
 */
...
function isValidExport(value: unknown): value is KxfLearningModulesExport {
  ...
  return (
    ...
    record.schema.startsWith('KXF-LEARNING-MODULES') &&
    Array.isArray(records?.learning_modules)
  );
}

// ── KG v0.2.0 → SSF shape normalization ──────────────────────────────────────
// KG uses camelCase (entryQuestion, depthMin, moduleUnlocks, noxiaGrants).
// SSF KxfLearningModule interface uses snake_case (entry_question, depth_min, etc.).
// This function bridges the two without changing either schema.
...
// ── Merge local modules ───────────────────────────────────────────────────────
// LOCAL_TMQ_MODULES take priority over KG data (they are SSF-local, not in KG).
// FALLBACK_MODULES fill in if KG is unreachable.
...
export async function fetchLearningModules(): Promise<KxfLearningModule[]> {
  try {
    ...
    if (!response.ok) {
      console.warn(`[SSF] kxf-learning-modules fetch failed: HTTP ${response.status} — using fallback`);
    ...
    if (!isValidExport(data)) {
      console.warn('[SSF] kxf-learning-modules: unexpected shape — using fallback');
      return mergeLocalModules(FALLBACK_MODULES);
    }
    ...
    console.warn('[SSF] kxf-learning-modules: fetch error — using fallback', err);
    return mergeLocalModules(FALLBACK_MODULES);
  }
}
...
// ── Subject config ────────────────────────────────────────────────────────────
// Covers all SSF SubjectCode values (see components/LearnMap/types.ts).
...
// ── Local TM2 modules (SSF-only, not in KG) ───────────────────────────────────
const emptyAssets = { image: [] as string[], video: [] as string[], audio: [] as string[], experiment: [] as string[] };

const LOCAL_TMQ_MODULES: KxfLearningModule[] = [
  {
    id: 'TM2-COMB-001', version: '0.1.0', created: '2026-07-04', modified: '2026-07-04T08:45:00+02:00',
    meta: { title: 'Überlagerung von Spannungen', subject: 'ENG', type: 'learning_path', status: 'built',
      entry_question: 'Wann darf man Spannungen addieren — und wann entsteht ein Spannungszustand?',
      depth_min: 3, depth_max: 4, duration_min: 60, duration_max: 60 },
  ...
  {
    id: 'TM2-PRESS-001', version: '0.1.0', created: '2026-07-04', modified: '2026-07-04T08:45:00+02:00',
    meta: { title: 'Dünnwandige Druckbehälter und Kesselformeln', subject: 'ENG', type: 'learning_path', status: 'built',
      entry_question: 'Warum ist die Umfangsspannung beim Kessel doppelt so groß wie die Längsspannung?',
      depth_min: 3, depth_max: 4, duration_min: 45, duration_max: 45 },
  ...
  {
    id: 'TM2-STRESS-001', version: '0.1.0', created: '2026-07-04', modified: '2026-07-04T08:45:00+02:00',
    meta: { title: 'Spannungszustand im Punkt', subject: 'ENG', type: 'learning_path', status: 'built',
      entry_question: 'Warum gibt es nicht die eine Spannung im Punkt?',
      depth_min: 3, depth_max: 4, duration_min: 75, duration_max: 75 },
  ...
  {
    id: 'TM2-PRINCIPAL-001', version: '0.1.0', created: '2026-07-04', modified: '2026-07-04T08:45:00+02:00',
    meta: { title: 'Hauptspannungen und Mohrscher Kreis', subject: 'ENG', type: 'learning_path', status: 'built',
      entry_question: 'In welcher Richtung wird die Normalspannung maximal?',
      depth_min: 4, depth_max: 4, duration_min: 90, duration_max: 90 },
  ...
  {
    id: 'TM2-COMB-002', version: '0.1.0', created: '2026-07-04', modified: '2026-07-04T08:45:00+02:00',
    meta: { title: 'Biegung und Torsion an Wellen', subject: 'ENG', type: 'learning_path', status: 'built',
      entry_question: 'Warum ist der Randpunkt einer Welle bei Biegung und Torsion kritisch?',
      depth_min: 4, depth_max: 4, duration_min: 75, duration_max: 75 },
  ...
  {
    id: 'TM2-STRAIN-001', version: '0.1.0', created: '2026-07-04', modified: '2026-07-04T08:45:00+02:00',
    meta: { title: 'Mehrachsige Dehnung, Hooke und DMS', subject: 'ENG', type: 'learning_path', status: 'built',
      entry_question: 'Wie kommt man experimentell von gemessenen Dehnungen zu Spannungen?',
      depth_min: 4, depth_max: 4, duration_min: 60, duration_max: 60 },
...
// ── Fallback modules (KG unreachable) ─────────────────────────────────────────
const FALLBACK_MODULES: KxfLearningModule[] = [
  {
    id: 'LRN:SSF:PHY-1101', version: '0.1.0', created: '2026-06-28', modified: '2026-07-02T18:00:00+02:00',
    meta: { title: 'Was die Welt aus sich macht', subject: 'PHY', type: 'learning_path', status: 'built',
      entry_question: 'Warum klingt eine Geige anders als eine Flöte — obwohl beide denselben Ton spielen?',
      depth_min: 0, depth_max: null, duration_min: 10, duration_max: 45 },
  ...
  {
    id: 'LRN:SSF:PHY-1102', version: '0.1.0', created: '2026-06-28', modified: '2026-07-02T18:00:00+02:00',
    meta: { title: 'Licht als Informationsträger', subject: 'PHY', type: 'learning_path', status: 'built',
      entry_question: 'Warum ist Licht das wichtigste Werkzeug der Astronomie — und was steckt alles darin?',
      depth_min: 0, depth_max: null, duration_min: 20, duration_max: 90 },
  ...
  {
    id: 'LRN:SSF:CHE-1101', version: '0.1.0', created: '2026-06-28', modified: '2026-07-02T18:00:00+02:00',
    meta: { title: 'Was Wasser so besonders macht', subject: 'CHE', type: 'learning_path', status: 'built',
      entry_question: 'Warum ist Wasser flüssig — obwohl H₂ und O₂ beide Gase sind?',
      depth_min: 0, depth_max: null, duration_min: 15, duration_max: 60 },
