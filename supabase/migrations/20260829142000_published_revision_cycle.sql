-- A published contribution may be reopened for a new reviewed version.
-- The existing publication snapshot remains immutable and public until a later approved revision is materialized.
...
begin
  ...
  v_allowed := case v_from
    ...
    when 'approved' then p_status = 'archived'
    when 'published' then p_status in ('revision_requested','archived')
    else false
  end;
  ...
  set
    ...
    editor_note = case when p_status in ('editorial_review','approved','rejected','archived') then nullif(trim(coalesce(p_note,'')), '') else editor_note end,
    reviewed_at = case when p_status in ('scientific_review','revision_requested','editorial_review') then now() else reviewed_at end,
    approved_at = case when p_status = 'approved' then now() when p_status = 'revision_requested' then null else approved_at end,
    updated_at = now()
  ...
  values (
    v_row.id,
    auth.uid(),
    case when v_from = 'published' and p_status = 'revision_requested' then 'new_version_requested' else 'editorial_transition' end,
    v_from,
    p_status,
...
comment on function public.ssf_editorial_transition_author_contribution(uuid, text, text) is
  'Controlled editorial state machine. Published contributions may be reopened for revision; the current immutable publication remains live until a later approved revision is materialized as the next version.';
