...
// Bottom fades to --bg. aria-hidden.
...
function buildKeyframes(index: number, maxOpacity: number): string {
  ...
  if (parseFloat(e) <= 100) {
    return `@keyframes hero-img-${index} {
      0% { opacity:0; } ${s}% { opacity:0; } ${pi}% { opacity:${maxOpacity}; }
      ${po}% { opacity:${maxOpacity}; } ${e}% { opacity:0; } 100% { opacity:0; } }`;
  }
  ...
  return `@keyframes hero-img-${index} {
    ...
    ${s}% { opacity:0; } ${pi}% { opacity:${maxOpacity}; }
    ${po}% { opacity:${maxOpacity}; } 100% { opacity:0; } }`;
}
...
export default function HeroBackground() {
  const css = `
    ...
    @keyframes node-pulse { 0%,100%{opacity:.2} 50%{opacity:.5} }
    @keyframes dot-pulse  { 0%,100%{opacity:.25;transform:scale(1)} 50%{opacity:.5;transform:scale(1.3)} }
  ...
  return (
    ...
    >
      ...
      >
        <defs>
          {/* Bottom fade: images dissolve into --bg */}
          <linearGradient id="hero-bottom-fade" x1="0" y1="0" x2="0" y2="1">
            <stop offset="55%" stopColor="#fbfaf7" stopOpacity="0" />
        ...
        {NODES.map((n, i) => (
          <circle key={`r${i}`} cx={n.x} cy={n.y} r={i % 5 === 0 ? 7 : 4}
        ...
        {NODES.map((n, i) => (
          <circle key={`d${i}`} cx={n.x} cy={n.y} r={i % 5 === 0 ? 2.5 : 1.5}
            fill="#b98b2d"
            style={{
        ...
        {/* Bottom fade overlay — images dissolve into page background */}
        <rect x="0" y="0" width="1200" height="600" fill="url(#hero-bottom-fade)" />
      </svg>
    </div>
  );
}
