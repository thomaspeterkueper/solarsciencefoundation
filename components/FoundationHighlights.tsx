...
export default async function FoundationHighlights({ locale = 'en', placement = 'foundation' }: { locale?: 'de' | 'en'; placement?: 'foundation' | 'home' }) {
  const items = await getFeaturedContributions(placement, placement === 'home' ? 3 : 6);
  const de = locale === 'de';
  ...
  return <section className="subject-section" style={{ maxWidth: 1000 }}>
    ...
    <h2 className="section-title">{home ? (de ? 'Neue ausgewählte Beiträge' : 'New selected contributions') : (de ? 'Redaktionell ausgewählte Beiträge' : 'Editorially selected contributions')}</h2>
