    ...
    <footer className="site-footer">
      <div className="ui-container ui-container-wide site-footer-inner">
        <div className="site-footer-brand">
          ...
          <p>{isGerman ? 'Eine unabhängige Plattform für wissenschaftliche Neugier — offen für alle, überall.' : 'An independent platform for scientific curiosity — open to everyone, everywhere.'}</p>
          <span className="site-footer-fiction">{isGerman ? 'Gegründet in Sundern 2045 · NOχ¹Δ Universe · Fiktives Wissenschaftsprojekt' : 'Founded in Sundern 2045 · NOχ¹Δ Universe · Fictional science project'}</span>
        </div>
        <nav className="site-footer-nav" aria-label={isGerman ? 'Fußnavigation' : 'Footer navigation'}>
          ...
          <div className="site-footer-group">
            ...
            <Link href={`${prefix}/impressum`}>{isGerman ? 'Impressum' : 'Imprint'}</Link>
            <Link href={`${prefix}/datenschutz`}>{isGerman ? 'Datenschutz' : 'Privacy'}</Link>
            <Link href={`${prefix}/nutzungsbedingungen`}>{isGerman ? 'Nutzungsbedingungen' : 'Terms'}</Link>
